...
interface NamespaceInfo {
  ...
  isRemote?: boolean;
  repository?: string;
  content?: string; // 添加内容字段，用于存储命名空间的代码内容
}

// 存储所有找到的命名空间
let foundNamespaces: NamespaceInfo[] = [];
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    disposable,
    remoteCheckDisposable,
    contentCheckDisposable,
...
function loadConfiguration() {
  ...
  UPDATE_THROTTLE_MS = config.get<number>('updateInterval', 500);
}
...
async function loadRemoteRepositories() {
  // 获取远程仓库配置
  const config = vscode.workspace.getConfiguration('namespaceChecker');
  const remoteRepos = config.get<string[]>('remoteRepos', []);
...
async function scanWorkspaceNamespacesAndContent() {
  ...
  // 获取配置的搜索路径
  const config = vscode.workspace.getConfiguration('namespaceChecker');
  const searchPaths = config.get<string[]>('searchPaths', ['src', 'include']);
...
// 查找指定目录下的所有C/C++文件
async function findCppFiles(directory: string): Promise<string[]> {
  const result: string[] = [];
...
// 检查当前文档中的命名空间和内容
function checkDocumentNamespacesAndContent(document: vscode.TextDocument) {
  // 清除当前文档的诊断信息
  ...
  // 检查是否有重复的代码内容
  const config = vscode.workspace.getConfiguration('namespaceChecker');
  const enableContentCheck = config.get<boolean>('enableContentCheck', true);
...
// 检查命名空间是否有重复
function checkNamespaceDuplicates(document: vscode.TextDocument, namespaces: NamespaceInfo[]) {
  // 创建诊断信息数组
  const diagnostics: vscode.Diagnostic[] = [];
  
  // 检查每个命名空间是否有重复
  for (const ns of namespaces) {
    const duplicates = foundNamespaces.filter(
      item => item.name === ns.name && (item.file !== ns.file || item.line !== ns.line)
    );
...
// 检查代码内容是否有重复
function checkContentDuplicates(document: vscode.TextDocument, namespaces: NamespaceInfo[]) {
  // 已有的诊断信息
  ...
  for (const ns of namespaces) {
    ...
    const contentDuplicates = foundNamespaces.filter(item => 
      item.content && 
...
// 获取命名空间范围
function getNamespaceRange(document: vscode.TextDocument, position: vscode.Position): vscode.Range | undefined {
  // 查找命名空间声明的范围
...
// 提取文档中的命名空间和内容
function extractNamespacesAndContent(document: vscode.TextDocument): NamespaceInfo[] {
  const foundInDocument: NamespaceInfo[] = [];
  ...
  while ((match = namespaceRegex.exec(text)) !== null) {
    ...
    // 添加到全局命名空间列表（如果不存在）
    if (!foundNamespaces.some(ns => 
      ns.name === namespaceInfo.name && 
...
// 提取命名空间内容
function extractNamespaceContent(text: string, startIndex: number): string {
...
// 从远程仓库获取代码并检查命名空间
async function checkWithRemoteRepository(repoUrl: string) {
  try {
    ...
    await vscode.window.withProgress({
      ...
      try {
        ...
        // 记录远程命名空间
        const remoteNamespaces: NamespaceInfo[] = [];
...
// 从文本内容中提取命名空间
function extractNamespacesFromText(text: string): { name: string; line: number; column: number }[] {
  const result: { name: string; line: number; column: number }[] = [];
...
// 解析仓库URL
function parseRepositoryUrl(url: string): { type: 'github' | 'gitlab'; owner: string; repo: string } | null {
  try {
...
// 获取仓库文件列表
async function getRepositoryFiles(repoInfo: { type: 'github' | 'gitlab'; owner: string; repo: string }): Promise<{ path: string; type: string }[]> {
  if (repoInfo.type === 'github') {
    ...
    return data
      ...
      .map((item: any) => ({ path: item.path, type: 'blob' }));
  }
...
// 获取文件内容
async function getFileContent(repoInfo: { type: 'github' | 'gitlab'; owner: string; repo: string }, filePath: string): Promise<string> {
  if (repoInfo.type === 'github') {
